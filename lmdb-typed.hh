...
#undef openMacro
  ...
  struct ReadonlyOperations
  {
    ...
    bool get(uint32_t id, T& t)
    {
      ...
      detail::serFromString(data.get<std::string>(), t);
      return true;
    }
    ...
    struct iter_t
    {
        ...
        d_end(end)
      {
        ...
        if(d_on_index) {
          ...
          detail::serFromString(d_data.get<std::string>(), d_t);
        }
        else
          detail::serFromString(d_id.get<std::string>(), d_t);
      }
      ...
      next:;
        ...
        if(rc == MDB_NOTFOUND) {
          ...
          if(d_on_index) {
            ...
            detail::serFromString(data.get<std::string>(), d_t);
          }
          else {
            if(filter && !filter(data))
              goto next;
  ...
  public:
    ...
    uint32_t put(const T& t, uint32_t id=0)
    {
      ...
      (*d_txn)->put(d_parent->d_main, id, detail::serToString(t), flags);
